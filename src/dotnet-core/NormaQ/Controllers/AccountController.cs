...
namespace NormaQ.Controllers
{
    public class AccountController : Controller
    {
          ...
          if (esOperario)
        {
            ...
            Email = usuario.Email,
            // Filtramos de la matriz de permisos solo los departamentos donde es Operario (Rol 5)
            DepartamentosOperario = permisos
            .Where(p => p.RolId == 5)
            .Select(p => p.DepartamentoId)
            .ToList()
    };
    ...
    await _redis.StringSetAsync($"SSO_TOKEN_{ssoToken}", jsonPayload, TimeSpan.FromSeconds(45));

    // Redirigir a Laravel (asegúrate de que el puerto sea el correcto para tu contenedor)
